...
static atomic<bool> GLOBAL_FOUND(false);
static atomic<bool> GLOBAL_TIMED(false);
static chrono::steady_clock::time_point GLOBAL_START;
static double GLOBAL_LIMIT=3200.0;

struct BranchResult{
 int third=0,fourth=0; bool completed=false,timed=false,found=false,verified=false;
 uint64_t nodes=0,prunes=0,qprunes=0,lbprunes=0; double wall=0.0; array<int,14>w{};
};

struct Search{
 int third,fourth; array<int,14>a{}; array<unsigned char,51>dc{};
 ...
 int cap(int mi,int d){int m=MODS[mi];return d==0?2*(100/m-1):2*(100/m);}
 bool timeout(){
 ...
 void dfs(int depth,int last,int evens){
   ...
   if(depth==14){
     int wrap=100-a[13];
     if(wrap<1||third-1>wrap){prunes++;return;}
     found=true;w=a;GLOBAL_FOUND.store(true);return;
   }
   int minx=last+1;
   int maxx=100-rem;
   maxx=min(maxx,101-third-(rem-1));
 ...
 BranchResult run(){
   ...
   if(add(1,1,d1,q1)){
     a[1]=1;vector<int>d2;vector<array<int,3>>q2;
     if(add(third,2,d2,q2)){
       a[2]=third;vector<int>d3;vector<array<int,3>>q3;
       if(add(fourth,3,d3,q3)){
         a[3]=fourth;dfs(4,fourth,1+(third%2==0)+(fourth%2==0));undo(fourth,d3,q3);
       }
       undo(third,d2,q2);
     }
     undo(1,d1,q1);
   ...
   BranchResult b; b.third=third;b.fourth=fourth;b.completed=!timed;b.timed=timed;b.found=found;b.verified=found&&verify(w);
   b.nodes=nodes;b.prunes=prunes;b.qprunes=qprunes;b.lbprunes=lbprunes;b.wall=chrono::duration<double>(chrono::steady_clock::now()-st).count();b.w=w;
   return b;
 }
};

static void write_json(const string&path,int third,const vector<BranchResult>&R,double wall){
 ...
 f<<"  \"normalization\": \"unit difference mapped to {0,1}; third selected element fixed; reflection enforces third-1 <= wrap\",\n";
 f<<"  \"third\": "<<third<<",\n";
 f<<"  \"fourth_range\": ["<<third+1<<", "<<91-third<<"],\n";
 f<<"  \"expected_fourth_branches\": "<<(91-2*third)<<",\n";
 f<<"  \"observed_fourth_branches\": "<<R.size()<<",\n";
 f<<"  \"completed_exhaustively\": "<<(all?"true":"false")<<",\n";
 f<<"  \"timed_out\": "<<((!all)?"true":"false")<<",\n";
 f<<"  \"witness_found\": "<<(any?"true":"false")<<",\n";
 f<<"  \"witness_verified\": "<<(ok?"true":"false")<<",\n";
 f<<"  \"aggregate_nodes_including_third_root\": "<<nodes<<",\n";
 f<<"  \"aggregate_prunes\": "<<pr<<",\n";
 f<<"  \"aggregate_quotient_prunes\": "<<qp<<",\n";
 f<<"  \"aggregate_lower_bound_prunes\": "<<lb<<",\n";
 f<<"  \"wall_seconds\": "<<setprecision(12)<<wall<<",\n";
 f<<"  \"coverage\": \"At depth 3 the original exact DFS considers every fourth selected element u from third+1 through 91-third. Each branch below fixes exactly one such u and then executes the identical exact recursion and pruning rules. Their disjoint union is the complete normalized third-element branch.\",\n";
...
int main(int argc,char**argv){
 if(argc<6){cerr<<"usage: THIRD LIMIT_SECONDS THREADS RESULT_JSON BRANCH_TSV\n";return 2;}
 int third=atoi(argv[1]);GLOBAL_LIMIT=atof(argv[2]);int nth=max(1,atoi(argv[3]));string jout=argv[4],tout=argv[5];
 if(third<2||third>45){cerr<<"third must be 2..45\n";return 2;}
 vector<int>us;for(int u=third+1;u<=91-third;u++)us.push_back(u);
 vector<BranchResult>R(us.size());atomic<size_t>next(0);GLOBAL_START=chrono::steady_clock::now();
 vector<thread>ts;
 for(int q=0;q<nth;q++)ts.emplace_back([&](){while(!GLOBAL_FOUND.load()&&!GLOBAL_TIMED.load()){size_t k=next.fetch_add(1);if(k>=us.size())break;Search s;s.third=third;s.fourth=us[k];R[k]=s.run();}});
 for(auto&t:ts)t.join();double wall=chrono::duration<double>(chrono::steady_clock::now()-GLOBAL_START).count();
 // Fill not-started records explicitly if a witness or global timeout stopped scheduling.
 for(size_t k=0;k<R.size();k++)if(R[k].fourth==0){R[k].third=third;R[k].fourth=us[k];R[k].completed=false;R[k].timed=GLOBAL_TIMED.load();}
 write_json(jout,third,R,wall);
 ofstream o(tout);o<<"third\tfourth\tcompleted\ttimed\tfound\tverified\tnodes\tprunes\tqprunes\tlbprunes\twall_seconds\n";
 for(auto&r:R)o<<r.third<<'\t'<<r.fourth<<'\t'<<r.completed<<'\t'<<r.timed<<'\t'<<r.found<<'\t'<<r.verified<<'\t'<<r.nodes<<'\t'<<r.prunes<<'\t'<<r.qprunes<<'\t'<<r.lbprunes<<'\t'<<setprecision(12)<<r.wall<<'\n';
 bool all=true,ok=false;for(auto&r:R){all&=r.completed;ok|=r.verified;}
 cout<<"third="<<third<<" branches="<<R.size()<<" all="<<all<<" witness="<<ok<<" wall="<<wall<<"\n";
