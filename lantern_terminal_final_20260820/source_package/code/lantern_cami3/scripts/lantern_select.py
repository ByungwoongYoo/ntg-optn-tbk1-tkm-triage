...
def main():
    ...
    if set(seqs)!=set(meta):raise ValueError(f'FASTA/metadata mismatch: {len(seqs)} vs {len(meta)}')
    ...
    if not samples:raise SystemExit('No mapping-evidence samples')
    w=cfg['weights'].copy()
    ...
    for cid,seq in seqs.items():
        ...
        t=sum(supports)
        nsrc=int(m.get('source_count',0) or 0)
        nasm=int(m.get('assembler_count',0) or 0)
        feats={
            'length':min(1,math.log1p(len(seq))/math.log1p(100000)),
            'source_consensus':min(1,nasm/3),
            'temporal_recurrence':t/max(1,len(samples)),
            'short_breadth':max(sb or [0]),
            'long_breadth':0 if a.ablation=='no_long' else max(lb or [0]),
            'long_spanning':0 if a.ablation=='no_long' else min(1,math.log1p(sp)/math.log1p(6))
        }
        score=sum(w[k]*feats[k] for k in w)
        scopes=set(filter(None,m.get('scopes','').split(','))); single='single' in scopes
        consensus=(nasm>=cfg['minimum_assembler_sources_for_consensus']) and a.ablation!='no_consensus'
