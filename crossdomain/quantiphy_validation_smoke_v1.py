...
def category(df):
    ...
    # Avoid pandas 3.x agg/string coercion corner cases from nullable numeric depth_info.
    return df[cols].apply(lambda row:'|'.join(str(x) for x in row.tolist()),axis=1)
...
def main():
    ...
    res={'n':len(df),'columns':list(df.columns),'dtypes':{c:str(df[c].dtype) for c in df.columns},'ground_truth_column':yc,'baselines':{}}
    ...
    V=TfidfVectorizer(ngram_range=(1,2),min_df=1,sublinear_tf=True).fit_transform(text);sim=cosine_similarity(V);np.fill_diagonal(sim,-1)
    ...
    res['baselines']['leave_one_video_out_text_nearest_neighbor_exploratory']=eval_pred(df,y,pred);(out/'nn_pairs.json').write_text(json.dumps(nn,indent=2),encoding='utf-8')
