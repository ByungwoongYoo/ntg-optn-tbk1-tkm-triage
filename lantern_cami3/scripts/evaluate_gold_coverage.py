...
def main():
 ...
 vals=[r['recovery_fraction'] for r in genomes];unique_covered=sum(gcov.values());chimeric=[r for r in chimera if r['cross_binid_chimera']=='true'];summary={'n_genomes':len(genomes),'mean_genome_recovery':sum(vals)/len(vals) if vals else 0,'median_genome_recovery':sorted(vals)[len(vals)//2] if vals else 0,'genomes_recovered_50':sum(r['recovered_50']=='true' for r in genomes),'genomes_recovered_90':sum(r['recovered_90']=='true' for r in genomes),'unique_truth_bp_covered':unique_covered,'accepted_alignment_bp':total_accepted_alignment_bp,'alignment_to_unique_truth_ratio':total_accepted_alignment_bp/unique_covered if unique_covered else None,'assembly_contigs_with_accepted_alignment':len(chimera),'cross_binid_chimeric_contigs':len(chimeric),'cross_binid_chimeric_bp':sum(r['length'] for r in chimeric),'min_identity':a.min_identity,'min_alignment':a.min_alignment,'truth_only_evaluation':True,'boundary':'Cross-BINID mappings are a conservative chimera proxy against the CAMI gold-standard assembly; they are not identical to the official MetaQUAST misassembly metric.'}
