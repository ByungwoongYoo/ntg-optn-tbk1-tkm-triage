...
def main():
 ...
 with open(out/'combined_truth.fasta','w') as allf,open(out/'truth_mapping.tsv','w',newline='') as mf:
  ...
  for gid in sorted(genomes):
   with open(refs/f'{safe_id(gid)}.fasta','w') as gf:
    for i,(sha,(orig,seq)) in enumerate(sorted(genomes[gid].items()),1):
     sid=f'{safe_id(gid)}|{i:05d}';write_record(gf,sid,seq);write_record(allf,sid,seq);w.writerow({'sequence_id':sid,'genome_id':gid,'original_id':orig,'length':len(seq),'sha256':sha,'samples':','.join(sorted(presence[gid]))})
 (out/'TRUTH_SUMMARY.json').write_text(json.dumps({'n_genomes':len(genomes),'n_sequences':sum(len(v) for v in genomes.values()),'samples':sorted({s for v in presence.values() for s in v}),'truth_access_boundary':'Gold truth is used only after truth-blind assembly and selection artifacts are frozen.'},indent=2)+'\n')
